function scr_taunt_storeVariables()
{
    tauntStored = 
    {
        state: (place_meeting(x, y, obj_secretTreasure) && state == (18 << 0)) ? (1 << 0) : state,
        hsp: hsp,
        prevHsp: prevHsp,
        vsp: vsp,
        prevVsp: prevVsp,
...
function do_grab(arg0 = state)
{
    ...
    if (inputBufferSlap > 0)
    {
        ...
        else if (global.playerCharacter == (0 << 0) && sprite_index != spr_player_PZ_suplexDash_bump)
        {
            if (floatyGrab > 0)
            {
