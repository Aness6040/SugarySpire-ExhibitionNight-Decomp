function __scribble_gen_1_model_limits_and_bezier_curves()
{
    ...
    if (_model_max_width < 0)
        _model_max_width = infinity;
    
    var _model_max_height = _element.__wrap_max_height - (_element.__padding_t + _element.__padding_b);
    
    if (_model_max_height < 0)
        _model_max_height = infinity;
    ...
    if (_bezier_do)
    {
        ...
        var _bezier_lengths = array_create(20, 0);
        var _x1 = undefined;
        var _y1 = undefined;
        var _x2 = 0;
        var _y2 = 0;
        var _dist = 0;
        var _bezier_param_increment = 0.05263157894736842;
        ...
        repeat (19)
        {
            ...
            _x1 = _x2;
            _y1 = _y2;
            _x2 = (3 * _inv_t * _inv_t * _t * _bx2) + (3 * _inv_t * _t * _t * _bx3) + (_t * _t * _t * _bx4);
            _y2 = (3 * _inv_t * _inv_t * _t * _by2) + (3 * _inv_t * _t * _t * _by3) + (_t * _t * _t * _by4);
            var _dx = _x2 - _x1;
            var _dy = _y2 - _y1;
        ...
        if (_model_max_width >= 0 && !is_infinity(_model_max_width))
            __scribble_trace("Warning! Maximum width (" + string(_model_max_width) + ") has been replaced with Bezier curve length (" + string(_dist) + "). Use -1 as the maximum width to turn off this warning");
        
        _model_max_width = _dist;
        _generator_state.__bezier_lengths_array = _bezier_lengths;
    }
    
    _element = _generator_state.__element;
    ...
    if (_line_height_max < 0)
        _line_height_max = infinity;
    
    var _line_spacing_raw = _element.__line_spacing;
    var _line_spacing_add, _line_spacing_multiply;
    ...
    if (is_string(_line_spacing_raw))
    {
        ...
        if (string_char_at(_line_spacing_raw, _length) == "%")
        {
            ...
            {
                _line_spacing_add = 0;
                _line_spacing_multiply = real(string_copy(_line_spacing_raw, 1, _length - 1)) / 100;
            }
            catch (_error)
            {
    ...
    {
        _line_spacing_add = _line_spacing_raw;
        _line_spacing_multiply = 0;
    }
