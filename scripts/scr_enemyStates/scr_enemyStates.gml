function state_enemyNormal()
{
    ...
    if (place_meeting_collision(x + image_xscale, y, (1 << 0)) || (place_meeting(x + image_xscale, y, obj_clutterCone) && !place_meeting(x, y, obj_clutterCone)))
    {
        scr_enemy_turn_trigger();
        exit;
    }
...
function state_enemyHit()
{
    ...
    with (create_afterimage((0 << 0), image_xscale))
    {
        image_alpha = 0.6;
        vanish = true;
    }
    ...
    if (hitHsp != 0 && (place_meeting_collision(x + sign(hitHsp), y, (1 << 0)) || place_meeting(x + sign(hitHsp), y, obj_hallway)) && !place_meeting(x + sign(hitHsp), y, obj_destructibles))
    {
        vsp = -5;
        impact_test = true;
    }
