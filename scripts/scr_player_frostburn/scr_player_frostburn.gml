...
function state_player_frostburnslide()
{
    ...
    if (place_meeting_collision(x + sign(hsp), y, (1 << 0)) && !place_meeting(x + sign(hsp), y, obj_destructibles) && !place_meeting(x + sign(hsp), y, obj_chocofrog) && !place_meeting(x + sign(hsp), y, obj_metalblock))
    {
        if (!grounded)
        {
            vsp = 0;
...
function state_player_frostburnjump()
{
    ...
    if (place_meeting_collision(x + sign(hsp), y, (1 << 0)))
    {
...
function state_player_frostburnstick()
{
    ...
    if (!place_meeting_collision(x + xscale, y, (1 << 0)) && sprite_index == spr_player_PZ_frostburn_wallCling)
    {
        sprite_index = spr_player_PZ_frostburn_walk_fall;
        xscale *= -1;
        movespeed = xscale * 5;
