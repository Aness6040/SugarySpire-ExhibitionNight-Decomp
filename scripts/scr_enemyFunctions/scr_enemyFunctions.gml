...
function scr_enemy_grabUpdatePosition(arg0)
{
    ...
    with (arg0)
    {
        if (!global.freezeframe && state != (0 << 0) && state != (17 << 0) && state != (46 << 0) && state != (4 << 0) && state != (15 << 0))
        {
            other.x = x;
            other.y = y;
            other.state = (4 << 0);
    ...
    yscale = (player_state == (15 << 0)) ? -1 : 1;
    
    if (player_state == (15 << 0))
    {
    ...
    if (player_state == (46 << 0) && state != (6 << 0))
    {
...
function scr_scareenemy()
{
    ...
    if (!jumpedFromBlock && scr_enemy_playerisnear(400, 130, undefined, 90, player_object) && collision_line(x, y, player_object.x, player_object.y, obj_solid, false, true) == -4 && (player_object.state == (33 << 0) || ((player_object.state == (41 << 0) && object_index != obj_fancypancake) && abs(movespeed) >= 12) || player_object.state == (42 << 0) || (player_object.movespeed >= 10 && player_object.state == (61 << 0))))
    {
        if (state != (3 << 0) && state != (6 << 0) && state != (4 << 0) && state != (5 << 0))
        {
            state = (3 << 0);
