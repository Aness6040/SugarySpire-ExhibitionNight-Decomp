...
function baddieOnPlayerCollisions()
{
    ...
    if (!invincibleBaddie && state != (5 << 0) && place_meeting(x, y, player_object) && !player_object.cutscene && player_object.state != (36 << 0))
    {
        if (baddieInvincibilityBuffer <= 0 && player_object.instakillmove && state != (6 << 0))
        {
            scr_instakillEnemy(id, player_object);
            
            with (player_object)
            {
                if (mach3Roll > 0 && state == (12 << 0))
                {
                ...
                if (!grounded && state != (37 << 0) && state != (12 << 0) && key_jump2)
                {
                ...
                if (state == (33 << 0) && sprite_index != spr_mach3hit)
                {
                    sprite_index = spr_mach3hit;
                    image_index = 0;
                }
                
                if ((state == (47 << 0) || state == (56 << 0) || state == (57 << 0)) && !grounded)
                {
                    vsp = min(-10, vsp);
                    jumpStop = true;
                    image_index = 0;
        ...
        if (baddieInvincibilityBuffer <= 0 && state != (6 << 0) && (player_object.state == (32 << 0) || player_object.state == (33 << 0) || player_object.state == (3 << 0) || player_object.state == (12 << 0)))
        {
        ...
        if (baddieInvincibilityBuffer <= 0 && player_object.state == (47 << 0) && player_object.sprite_index == spr_player_PZ_werecotton_drill_h)
        {
            event_play_oneshot("event:/SFX/player/punch", x, y);
        ...
        if (canBeStomped && vsp >= 0 && (player_object.state == (24 << 0) || player_object.state == (17 << 0)) && player_object.vsp > 0 && player_object.y < y && player_object.sprite_index != player_object.spr_stompprep)
        {
        ...
        if (canBeGrabbed && player_object.state == (10 << 0))
        {
            state = (5 << 0);
        ...
        if (canBeGrabbed && player_object.state == (8 << 0))
        {
            state = (5 << 0);
...
{
    if (state != (6 << 0) || !place_meeting(x, y, obj_parent_enemy))
        exit;
    
    with (instance_place(x, y, obj_parent_enemy))
    {
        if (!invincibleBaddie && state != (5 << 0))
        {
            instance_destroy();
            exit;
        }
...
{
    if (baddieInvincibilityBuffer > 0 || invincibleBaddie || state == (5 << 0) || !place_meeting(x, y, obj_instakillHitbox))
        exit;
